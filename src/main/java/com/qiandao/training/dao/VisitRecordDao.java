...
public interface VisitRecordDao extends BaseMapper<VisitRecordEntity> {
    ...
    @Select("<script>" +
            "SELECT DATE_FORMAT(create_time, '%Y-%m-%d %H:00') as time, " +
            "COUNT(*) as total, " +
    ...
    @Select("<script>" +
            "SELECT YEARWEEK(create_time, 1) as week, " +
    ...
    @Select("<script>" +
            "SELECT COUNT(*) as total, SUM(is_unique) as unique_count " +
            "FROM visit_record " +
            "WHERE map_id = #{mapId}" +
            "</script>")
    ...
    @Select("<script>" +
            "SELECT vr.*, m.url as original_url, m.random_url, m.create_time as map_create_time " +
